...
public partial class CustomInstallerProjectViewModel : ViewModelBase
{
    ...
    [ObservableProperty] private string scriptPreview =
        "Add project files and actions to preview the generated Chocolatey install script.";
    ...
    public CustomInstallerProjectViewModel(
        ...
        CustomInstallerProject project,
        string projectPath,
    ...
    [
        new(InstallActionKind.CopyFile, "Copy file", "Copy a bundled project file to an install path."),
        new(InstallActionKind.RunFile, "Run file", "Launch a bundled or literal file with arguments.")
    ];
    ...
    private async Task SaveProjectAsAsync()
    {
        ...
        if (!Path.GetExtension(path)
                .Equals(_customInstallerProjectStore.FileExtension, StringComparison.OrdinalIgnoreCase))
            path += _customInstallerProjectStore.FileExtension;
    ...
    private async Task AddProjectFileAsync()
    {
        ...
        try
        {
            ...
            File.Copy(selectedPath, destinationPath, false);
    ...
    private async Task BuildProjectAsync()
    {
        ...
        try
        {
            await SaveProjectToAsync(ProjectPath, false);
            var result =
                await _packageGenerator.PackCustomProjectAsync(
                    new CustomProjectPackRequest(ProjectPath, OutputDirectory));
    ...
    public void MoveActionBefore(ActionItemViewModel? dragged, ActionItemViewModel? target)
    {
        ...
        MarkDirty();
        RefreshScriptPreview();
        if (_settings.VerboseStatus)
            _status.SetSuccess("Action moved.", $"Moved action to position {targetIndex + 1}.");
    }
    ...
    {
        await _customInstallerProjectStore.SaveAsync(path, CreateProject());
        ProjectPath = path;
        IsDirty = false;
        RefreshScriptPreview();
    ...
    private CustomInstallerProject CreateProject()
    {
        return new CustomInstallerProject
        {
            Package = new PackageMetadata
            {
                Name = PackageName.Trim(), Version = Version.Trim(), Maintainer = Maintainer.Trim(),
                Description = Description.Trim()
            },
            Files = ProjectFiles.Select(file => new ProjectFile
                    { Id = file.Id.Trim(), SourcePath = file.SourcePath.Trim(), PackagePath = file.PackagePath.Trim() })
                .ToList(),
    ...
    private static InstallAction CreateAction(ActionItemViewModel item)
    {
        ...
        {
            Kind = item.Kind, Arguments = item.Arguments.Trim(), WaitForExit = item.WaitForExit,
            ValidExitCodes = ParseExitCodes(item.ValidExitCodesText), Overwrite = item.Overwrite
        };
    ...
    private void LoadProject(CustomInstallerProject project)
    {
        _loading = true;
        ...
        {
            var item = new ProjectFileItemViewModel
                { Id = file.Id, SourcePath = file.SourcePath, PackagePath = file.PackagePath };
        ...
        foreach (var action in project.Actions)
        {
            ...
            {
                Kind = action.Kind, Arguments = action.Arguments, WaitForExit = action.WaitForExit,
                ValidExitCodesText = string.Join(",", action.ValidExitCodes), Overwrite = action.Overwrite,
                IsExpanded = false
            };
    ...
    private void TrackProjectFile(ProjectFileItemViewModel file)
    {
        ...
        {
            if (args.PropertyName is nameof(ProjectFileItemViewModel.Id)
                or nameof(ProjectFileItemViewModel.PackagePath)) MarkDirty();
            RefreshScriptPreview();
        };
    }
    ...
    private ActionItemViewModel CreateDefaultAction(InstallActionKind kind)
    {
        ...
        return kind == InstallActionKind.CopyFile
            ...
            {
                Kind = InstallActionKind.CopyFile, SourceKind = ActionPathKind.PackageFile, SourceValue = firstFileId,
                DestinationKind = ActionPathKind.Literal, Overwrite = true
            }
            : new ActionItemViewModel
            {
                Kind = InstallActionKind.RunFile, SourceKind = ActionPathKind.PackageFile, SourceValue = firstFileId,
                WaitForExit = true, ValidExitCodesText = "0"
            };
    }
    ...
    private void RefreshScriptPreview()
    {
        try
        {
            ScriptPreview = _customInstallerScriptGenerator.Generate(CreateProject());
        }
        catch (Exception ex)
        {
    ...
    private static List<int> ParseExitCodes(string value)
    {
        var result = value.Split([',', ';', ' '], StringSplitOptions.RemoveEmptyEntries)
            .Select(part =>
                int.TryParse(part, NumberStyles.Integer, CultureInfo.InvariantCulture, out var code) ? code : 0)
            .ToList();
...
public partial class ActionItemViewModel : ObservableObject
{
    ...
    partial void OnDestinationValueChanged(string value)
    {
        OnPropertyChanged(nameof(Summary));
    }
