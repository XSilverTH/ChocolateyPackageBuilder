...
public sealed class PackageGenerator(
    ...
    : IPackageGenerator
{
    public async Task<PackageBuildResult> GenerateAsync(PackageBuildRequest request)
    {
    ...
    public PackScaffoldResult PackScaffold(string directoryPath, string? outputDirectory = null)
    {
    ...
    public async Task<PackCustomProjectResult> PackCustomProjectAsync(CustomProjectPackRequest request,
        CancellationToken cancellationToken = default)
    {
        ...
        var outputPath = Path.Combine(request.OutputDirectory,
            $"{project.Package.Name}.{project.Package.Version}.nupkg");
        await Task.Run(() =>
        {
            using var outFs = new FileStream(outputPath, FileMode.Create, FileAccess.Write, FileShare.None);
            builder.Save(outFs);
    ...
    private Task<PackageBuildResult> GeneratePackageAsync(PackageBuildRequest request, string installerFileName,
    ...
    private async Task<PackageBuildResult> GenerateScaffoldAsync(PackageBuildRequest request, string installerFileName,
        string scriptContent)
    {
        ...
        await using (var sourceStream = new FileStream(request.InstallerPath, FileMode.Open, FileAccess.Read,
                         FileShare.Read,
                         81920, true))
        await using (var destinationStream =
                     new FileStream(destinationPath, FileMode.Create, FileAccess.Write, FileShare.None, 81920, true))
        {
    ...
    private string CreateNuspec(PackageBuildRequest request)
    {
    ...
    private void ValidateRequest(PackageBuildRequest request)
    {
    ...
    private Dictionary<string, ResolvedProjectFile> ValidateProjectFiles(CustomInstallerProject project,
        string projectRoot)
    {
    ...
    private string ResolveProjectSourcePath(string projectRoot, ProjectFile file)
    {
    ...
    private string NormalizePackagePath(ProjectFile file)
    {
    ...
    private void ValidateActions(CustomInstallerProject project, IEnumerable<string> projectFileIds)
    {
    ...
    private void ValidateActionPath(ActionPath path, HashSet<string> projectFileIds)
    {
    ...
    private void ThrowIfWhiteSpace(string value, string fieldName)
    {
